...
один раз. Дальше два сигнала держат это в равновесии, чтобы не засорять
реальный диалог повторными «занят»:
  - Алишер прочитал чат, но сам ничего не написал — автоответ разрешён
    снова на следующее сообщение (не должно выглядеть, что игнорируют).
...
async def main() -> None:
    ...
    async def on_manual_reply(event):
        # Настоящий ответ (не команда в Избранном себе) — считаем чат
        # разобранным, автоответ на него больше не лезет, пока Алишер не
        # прочитает новое сообщение там и снова промолчит.
        if not event.is_private or event.chat_id == me.id:
            return
        already_replied.add(event.chat_id)
