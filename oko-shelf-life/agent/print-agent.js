...
const AGENT_VERSION = "2026-09-23.5";
...
// SELFTEST / SELFTEST PATTERN — штатные команды прошивки TSC-принтеров,
// но реальный тест (2026-09-23) показал, что этот дешёвый клон их не
// поддерживает: обе команды дали пустые этикетки без единой отметки.
// Оставлены (вдруг на другом экземпляре/прошивке заработают — тогда это
// будет видно сразу), но полагаться на них нельзя.
//
// Поэтому основная, гарантированно интерпретируемая любым TSPL-клоном
// проверка головки — залить всю этикетку СПЛОШНЫМ чёрным прямоугольником
// командой BAR (а не текстом — тест шрифта уже показал блёклый результат
// независимо от шрифта/плотности, здесь нужен независимый способ понять,
// дело в головке или в чём-то другом). Если голова исправна — выйдет
// ровный чёрный прямоугольник без пропусков. Если на одном и том же
// месте на каждой копии виден белый вертикальный пропуск на всю высоту
// — это физически повреждённый нагревательный элемент, головка требует
// ремонта/замены. 464x240 точек — примерно 58x30мм при 8 точек/мм
// (203dpi); если реальная плотность точек на этом принтере другая,
// прямоугольник может не дотянуть до самого края этикетки, но для
// проверки головки это не важно — важна ровность заливки.
function buildPrinterSelfTestPayload() {
  const lines = ["SELFTEST", "SELFTEST PATTERN", "SIZE 58 mm,30 mm", "GAP 3 mm,0 mm", "DENSITY 15", "CLS", "BAR 0,0,464,240", "PRINT 1", ""];
  return Buffer.from(lines.join("\r\n"), "ascii");
}
...
async function runPrinterSelfTest() {
  ...
  const text = outcome.success
    ? "Команда отправлена. Должно выйти 2-3 этикетки: сначала попытка SELFTEST (настройки/узор из прошивки — этот клон, скорее всего, их не поддержит, пустая этикетка тут не страшно), а ПОСЛЕДНЯЯ этикетка — сплошной чёрный прямоугольник, это и есть главная проверка головки. Смотрите именно на неё: залито ровно и полностью — голова исправна, дело не в ней. Если на одном и том же месте на каждой копии виден белый вертикальный пропуск на всю высоту — это повреждённый элемент головки, нужен ремонт/замена."
...
<div class="wrap">
  ...
  <div class="card" style="margin-top: 24px;">
    ...
    <p style="font-size: 12px; color: #8a8272; margin: 8px 0 0;">Пробует служебные команды прошивки принтера (могут не сработать на дешёвом клоне), а ПОСЛЕДНЯЯ этикетка — сплошной чёрный прямоугольник для проверки головки. Смотрите на неё: залито ровно и полностью — голова исправна. Постоянный белый пропуск на одном месте — голова требует ремонта.</p>
  </div>
</div>
</body>
</html>`;
