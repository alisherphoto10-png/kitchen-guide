...
const AGENT_VERSION = "2026-09-23.6";
...
// БЫЛО: SELFTEST / SELFTEST PATTERN перед BAR — но реальный тест
// (2026-09-23) с обеими командами в одном задании дал ПУСТУЮ этикетку —
// даже гарантированно понятный любому TSPL-клону BAR не пропечатался.
// Похоже, непонятные клону команды SELFTEST сбивают разбор всего, что
// идёт следом в том же задании, и BAR тоже пропадает. Убрал SELFTEST
// отсюда полностью — чтобы проверить голову БЕЗ риска зависимостей от
// неподдерживаемых команд, шлём только чистый, изолированный BAR.
//
// BAR заливает всю этикетку СПЛОШНЫМ чёрным прямоугольником (а не
// текстом — тест шрифта уже показал блёклый результат независимо от
// шрифта/плотности, здесь нужен независимый способ понять, дело в
// головке или в чём-то другом). Если голова исправна — выйдет ровный
// чёрный прямоугольник без пропусков. Если на одном и том же месте на
// каждой копии виден белый вертикальный пропуск на всю высоту — это
// физически повреждённый нагревательный элемент, головка требует
...
function buildPrinterSelfTestPayload() {
  const lines = ["SIZE 58 mm,30 mm", "GAP 3 mm,0 mm", "DENSITY 15", "CLS", "BAR 0,0,464,240", "PRINT 1", ""];
  return Buffer.from(lines.join("\r\n"), "ascii");
}
...
async function runPrinterSelfTest() {
  ...
  const text = outcome.success
    ? "Команда отправлена. Должна выйти ОДНА этикетка — сплошной чёрный прямоугольник, это проверка головки. Залито ровно и полностью — голова исправна, дело не в ней. Если на одном и том же месте на каждой копии виден белый вертикальный пропуск на всю высоту — это повреждённый элемент головки, нужен ремонт/замена. Если снова пусто — сообщите, это уже не про шрифт/плотность, будем разбираться иначе."
...
<div class="wrap">
  ...
  <div class="card" style="margin-top: 24px;">
    ...
    <p style="font-size: 12px; color: #8a8272; margin: 8px 0 0;">Печатает сплошной чёрный прямоугольник на всю этикетку — проверка головки. Залито ровно и полностью — голова исправна. Постоянный белый пропуск на одном месте — голова требует ремонта.</p>
  </div>
</div>
</body>
</html>`;
