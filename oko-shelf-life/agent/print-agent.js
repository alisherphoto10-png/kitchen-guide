...
const AGENT_VERSION = "2026-09-23.8";
...
// Типографские символы, которых нет в таблице выше, но которые реально
// приходят в строках чеков (тикеты заказов печатают "Заказ — Облако",
// "Сет — 2 шт." — на ленте было "Заказ ? Облако"). Заменяем на ASCII-аналоги,
// а не на коды CP866: em dash в CP866 нет вовсе, а псевдографику/≈/№ из
// верхней половины таблицы на этом принтере никто не проверял — ASCII
// печатается гарантированно. Тот же список, что у печати ТТК на сервере
// (src/utils/ttkPrint.js), чтобы любой чек выглядел одинаково.
...
function textToCp866(text) {
  ...
  for (const raw of text) {
    const ch = ASCII_FALLBACK[raw] !== undefined ? ASCII_FALLBACK[raw] : raw;
    if (ch.length !== 1) {
      for (const c of ch) bytes.push(c.charCodeAt(0));
      continue;
    }
...
// БЫЛО: команда BAR (сплошная заливка) — что с SELFTEST перед ней, что
// БЕЗ него (изолированно) дала ПУСТУЮ этикетку оба раза. Но при этом
// обычный TEXT тем же самым каналом много раз печатал строку
// "TEST 58x30mm" ЧЁТКО и стабильно. Если бы голова была физически
// повреждена — TEXT бы тоже не печатал, а он печатает. Значит дело не
// в голове, а в самой команде BAR: либо этот клон её не поддерживает,
// либо синтаксис (x,y,width,height) для него не тот. Вместо неё —
// проверяем голову командами, которые УЖЕ ПОДТВЕРЖДЕНО работают на
// этом принтере: несколько строк TEXT тем же шрифтом "2", что дал
// чистый результат, плотно заполненных повторяющимся символом "X", на
// всю высоту этикетки. Не идеальная сплошная заливка (между строками
// остаются просветы), но достаточно плотно, чтобы увидеть провал в
// конкретном месте головки, если он есть, и при этом не зависит от
// непроверенной команды.
function buildPrinterSelfTestPayload() {
  const row = "X".repeat(35);
  const lines = ["SIZE 58 mm,30 mm", "GAP 3 mm,0 mm", "DENSITY 15", "CLS"];
  for (let y = 10; y <= 200; y += 25) {
    lines.push(`TEXT 5,${y},"2",0,1,1,"${row}"`);
  }
  lines.push("PRINT 1", "");
  return Buffer.from(lines.join("\r\n"), "ascii");
}
...
async function runPrinterSelfTest() {
  ...
  const text = outcome.success
    ? "Команда отправлена. Должна выйти ОДНА этикетка, плотно заполненная строками из букв X — это проверка головки уже проверенным шрифтом (тем же, что раньше давал чёткий результат). Все строки читаются ровно, без провалов — голова исправна. Если в одном и том же месте на каждой строке видна пустота — повреждён элемент головки, нужен ремонт/замена. Если снова пусто — сообщите, будем разбираться иначе."
...
<div class="wrap">
  ...
  <div class="card" style="margin-top: 24px;">
    ...
    <p style="font-size: 12px; color: #8a8272; margin: 8px 0 0;">Плотно заполняет этикетку строками "X" уже проверенным шрифтом — проверка головки без риска от непроверенных команд. Все строки ровные, без провалов — голова исправна. Постоянная пустота в одном месте на каждой строке — голова требует ремонта.</p>
  </div>
</div>
</body>
</html>`;
