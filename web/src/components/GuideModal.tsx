// Гид для владельцев — всё в одном окне администратора платформы:
// ссылка (одна настройка; пустое поле скрывает гид везде: вход, меню, профиль
// мини-аппа, /guide в ботах), свои фото вместо заглушек блюд на странице /guide/
// и список «Частые вопросы». Фото и вопросы — данные, страница гида подхватывает
// их сразу (GET /api/guide/content), без пересборки.
import { useEffect, useRef, useState } from 'react'
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query'
import { ArrowDown, ArrowUp, BookMarked, ExternalLink, ImagePlus, Pencil, Plus, RotateCcw, Trash2 } from 'lucide-react'
...
export function GuideModal({ onClose }: { onClose: () => void }) {
  return (
    <Modal wide title={<span className="flex items-center gap-2"><BookMarked className="h-4 w-4 text-brand" />Гид для владельцев</span>} onClose={onClose}>
...
function FaqSection() {
  ...
  return (
    <section className="grid gap-3">
      <div className="flex items-center gap-2">
        <h3 className="font-bold flex-1">Частые вопросы</h3>
        {editing !== 'new' && <button type="button" className="btn-outline btn-sm" onClick={() => setEditing('new')}><Plus className="h-3.5 w-3.5" />Вопрос</button>}
      </div>
